...
def render_markdown(report: AuditReport) -> str:
    ...
    for severity in [Severity.CRITICAL, Severity.HIGH, Severity.MEDIUM, Severity.LOW, Severity.INFO]:
        ...
        for finding in findings:
            ...
            lines.append("")
            lines.append(
                f"**Category:** {finding.category.value} | **Confidence:** {finding.confidence:.0%} | **Potential Savings:** ${finding.potential_savings:,.2f}"
            )
            lines.append("")
    ...
    if report.action_items:
        ...
        for i, item in enumerate(report.action_items, 1):
            lines.append(
                f"| {i} | {item.title} | {item.priority.value} | ${item.estimated_savings:,.2f} | {item.effort} |"
            )
        lines.append("")
    ...
    if has_intel:
        ...
        if intel.benfords_summary:
            score_label = (
                f" (conformity: {intel.benfords_conformity_score:.0%})"
                if intel.benfords_conformity_score is not None
                else ""
            )
        ...
        if intel.cashflow_summary:
            runway_label = (
                f" ({intel.cashflow_runway_months:.1f} months runway)" if intel.cashflow_runway_months else ""
            )
    ...
    lines.append("---")
    lines.append(
        "*Report generated by [FiscalPilot](https://github.com/meetpandya27/FiscalPilot) — The Open-Source AI CFO*"
    )
