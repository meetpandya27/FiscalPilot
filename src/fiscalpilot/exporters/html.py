    ...
    for severity in severity_order:
        ...
        for finding in severity_findings:
            ...
            if finding.evidence:
                ...
                evidence_html = (
                    f'<ul style="margin: 0.5rem 0; padding-left: 1.5rem; color: var(--gray-600);">{evidence_items}</ul>'
                )
...
def _render_proposed_actions_html(proposed_actions: list) -> str:
    ...
    for action in proposed_actions:
        ...
        if action.steps:
            step_items = "".join(
                f'<li style="margin-bottom: 0.5rem;">{_escape(step.description)}</li>' for step in action.steps[:10]
            )
            steps_html = f"""
