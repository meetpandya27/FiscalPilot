...
class MenuEngineeringAnalyzer:
    ...
    def _generate_recommendations(
        ...
        if plowhorses:
            total_plowhorse_volume = sum(p.quantity_sold for p in plowhorses)
        ...
        # Dog recommendations
        if dogs:
            total_dog_loss = sum(d.total_profit for d in dogs if d.total_profit < 0)
    ...
    def _generate_insights(
        ...
        # Puzzle opportunity
        if puzzles:
            puzzle_margin_total = sum(p.contribution_margin * p.quantity_sold for p in puzzles)
            if len(puzzles) >= 2:
                insights.append(
